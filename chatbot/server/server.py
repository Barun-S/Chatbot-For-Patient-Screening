...
@app.route('/addflowapi', methods=['POST'])
def addflowapi():
    ...
    for item in data['questions']:
        question = {}
        question['ques'] = item['question']
        condition = {}
        for nested in item['nestedQuestions']:
            nested_question = nested['nestedQuestion']
            nested_condition = nested['condition']
            if nested_condition in condition:
                condition[nested_condition].append(nested_question)
            else:
                condition[nested_condition] = [nested_question]
        question['condition'] = condition
        questions.append(question)
    output = {"symptom": topic, "questions": questions}
    # print(output)
    sym_collection = db["chatbot_dataset"]
    sym_collection.insert_one(output)
    return jsonify(output="Thank You! Your responses are recorded.")
    
@app.route('/fetchalldata', methods=['GET'])
def fetchalldata():
    sym_collection = db["chatbot_dataset"]
    sym_data = list(sym_collection.find({}, {'_id': 0}))
    # print(sym_data)
    return jsonify(sym_data)
...
@app.route('/graphapi', methods=['POST'])
def process_url():
    json = request.get_json()
