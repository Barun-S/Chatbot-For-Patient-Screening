...
OBJ.prototype = {
    ...
    initSpehere: function (gl, rings, segments, radius, baseColor)
	{
		...
		for(var i =0; i< indexData.length; i+=3){
			VertexArray.push(indexData[i]);
			VertexArray.push(indexData[i+1]);
			VertexArray.push(indexData[i+2]);
		}
		...
		this.initBuffers(vertexPositionData, normalData, textureCoordData, vertexLineNormal, indexData, LinesArray, VertexArray, indexLineNormal);
	},

	loadModel: function(data){
		var mesh = parseOBJ(data, false);

		this.Vertex = [];
		this.Textures = [];
		this.Triangles = [];
		
		for(var i=0;i<mesh.vertexPositionData.length; i +=3){
			this.Vertex.push(new Vector3D(mesh.vertexPositionData[i],mesh.vertexPositionData[i+1], mesh.vertexPositionData[i+2]));
		}

		for(var i=0;i<mesh.textureCoordData.length; i +=2){
			this.Textures.push(new Vector3D(mesh.textureCoordData[i],mesh.textureCoordData[i+1]));
		}

		for(var i=0;i<mesh.indexData.length; i +=3){
			this.Triangles.push(new Vector3D(mesh.indexData[i],mesh.indexData[i+1], mesh.indexData[i+2]));
		}
			

		this.initBuffers(mesh.vertexPositionData, mesh.normalData, mesh.textureCoordData, 
					mesh.vertexLineNormal, mesh.indexData, mesh.LinesArray, 
					mesh.VertexArray, mesh.indexLineNormal);
	},

	initBuffers: function(vertexPositionData, normalData, textureCoordData, vertexLineNormal, indexData, LinesArray, VertexArray, indexLineNormal){
		//Data size
	...
	loadFromImage: function(gl,image){

		this.textureSize[0] = image.width;
		this.textureSize[1] = image.height;
	...
	RendertoTexture: function(gl, quad, shader, windowswidth, windowsheight, x, y, blurTexture){
		...
		gl.bindTexture(gl.TEXTURE_2D, blurTexture.texture);

	    var trans = Matrix.glTranslate($V([-0.5, -0.5, 0.0]));
	    var scale = Matrix.glScale($V([this.textureSize[0] * 0.05, this.textureSize[1] * 0.05, 1.0]));
	    ...
	    var color = hexToRgb(blurTexture.brushColor);
	    gl.uniform3f(shader.getUniformLoc("color"), color.r/255, color.g/255, color.b/255);
	    gl.uniform1f(shader.getUniformLoc("intensity"), blurTexture.intensity);
